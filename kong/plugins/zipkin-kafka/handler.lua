...
local zipkin_codec = require "kong.plugins.zipkin-kafka.codec"
...
ZipkinLogHandler.PRIORITY = 10000
...
local function log(premature, reporter)
	...
	kong.log.notice("reporter flush ","#####")
	if not ok then
...
function ZipkinLogHandler:log(conf)
	...
	kong.log.notice("failed to create timer: ","*****")
	if not ok then
